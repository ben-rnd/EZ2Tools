...
void convertToAbm(LPWSTR ezFileName) {
    if (endsWith(ezFileName, L".bmp")) {
        ...
        memcpy((BYTE*)lpAddress + 0x2, (BYTE*)lpAddress + 0x4, sizeof(UINT8));
        ...
        memcpy((BYTE*)lpAddress + 0x7, (BYTE*)lpAddress + 0x13, sizeof(UINT8));
        memcpy((BYTE*)lpAddress + 0x9, (BYTE*)lpAddress + 0x17, sizeof(UINT8));

        DWORD* dataAtOffset = (DWORD*)((BYTE*)lpAddress + 0xA);  // calculate the address of data at the offset
        DWORD originalValue = *dataAtOffset;  // store the original value
        *dataAtOffset = *dataAtOffset ^ 0x109a;  // perform XOR operation with 0x109a
